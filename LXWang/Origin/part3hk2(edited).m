...
%if shname{mm,1}(3) == '0'
    ...
    fname2=['D:\Softwares\Matlab2015\bin\Codes\LXWang\Origin\' fname];
%end;
%if shname{mm,1}(3) == '6'
...
for k=1:N
    if v00(k)~=0
        ...
        if datenum(dir{i},'mm/dd/yyyy')==datenum('2013/12/31')
            n2=i;
        end;
        if datenum(dir{i},'mm/dd/yyyy')==datenum('2011/01/03')
            n1=i;
        end;
        i=i+1;
    end;
...
%n2=N-n0;
%2011/01/03 往前245个交易日，到2013/12/31
for k=n1-245:n2
    p(k)=p000(k);
end;
%ns number,used to get average;pm abs return sum;pv variance;p4 return ^4
ns=0;pm=0;pv=0;p4=0;
for k=n1+1:n2
    %r index starts from n1+1
    r(k)=log(p(k)/p(k-1));
    if abs(r(k))<0.1
        ns=ns+1;
        pm=pm+abs(r(k));
...
for k=n1:n2-1
    %a6 in paper, goes negative if seller exists
    aaup(k)=aa(1,1,k);
    %ed7 in paper, for buyer
    aadn(k)=aa(2,1,k);
    az(k)=0;
...
%only mood is used, those aaupp,aaupn, etc. were not used.
for k=n1+5:n2-1
    avmood(k)=0;
    for i=1:5
