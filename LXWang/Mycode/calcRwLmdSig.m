function resultArr=calcRwLmdSig(lmd,DatePrice,startI,endI,Nsig,avgPxN,avgMoodN,bigR)
...
%resultArr=[totalR,bnhR,nb,RoSM,RoSBH,minMR,minBR];
 ...
 if (((m+startI)<2)||DatePrice(m+startI-1,9)==0)
        totalR=0;
        bnhR=1;
        nb=0;
        RoSM=0;
     ...
     c=Nsig*DatePrice(m+startI-1,9);
    if((m+endI)<avgMoodN)||((m+startI)<1)
        totalR=0;
        bnhR=1;
        nb=0;
        RoSM=0;
